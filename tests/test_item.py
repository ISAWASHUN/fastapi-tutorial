...
def test_find_by_id_異常系(client_fixture: TestClient):
  ...
  item = response.json()
  assert item["detail"] == "Item not Found"
...
def test_create(client_fixture: TestClient):
  response = client_fixture.post("/items", json={"name": "SP", "price": 30000, "user_id": 1})
...
def test_update_正常系(client_fixture: TestClient):
  response = client_fixture.put("/items/1",json={"name": "SP", "price": 20000})
  assert response.status_code == 200
  item = response.json()
  assert item["name"] == "SP"
...
def test_update_異常系(client_fixture: TestClient):
  response = client_fixture.put("/items/10",json={"name": "SP", "price": 20000})
  assert response.status_code == 404
